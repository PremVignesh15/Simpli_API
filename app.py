...
app = FastAPI(
    ...
    description="Multilingual text simplification (English, Hindi, Tamil, Urdu) with grammar correction",
    version="2.1.0"
)
...
def simplify_generic(text: str, mapping: dict, ignore_case: bool = True) -> str:
    ...
    for hard, easy in mapping.items():
        if ignore_case:
            pattern = re.compile(re.escape(hard), flags=re.IGNORECASE)
        else:
            pattern = re.compile(re.escape(hard))
...
def detect_language(text: str) -> str:
    # Very simple heuristic – you already had something similar
    for ch in text:
        code = ord(ch)
...
tool_hi = language_tool_python.LanguageTool('hi-IN')
...
def root():
    return {
        "message": "Text Simplification API with Grammar Correction",
        "status": "running",
        "version": "2.1.0",
        "endpoints": {
            "simplify": "/simplify"
